...
public class WildcardMatching {
        ...
        boolean isMatch(int i, int j, int n1, int n2, String s, String p) {
            ...
            //if 1st string is finished but 2nd is still left
            //then we return true of remaining all are * in p else false
            if(i>=n1) {
            ...
            //wildcard matching, 2 cases,
            //case 1: ignore the wildcard and move to next char eg->s="abcdef" p="abc*def"
            ...
            //2 char, 3 char etc by keeping j same and i increase by 1, so no for loop is needed
            //i think this is main crux of the problem.
            else if(s.charAt(i)!=p.charAt(j) && p.charAt(j)=='*') {
                return dp[i][j] = isMatch(i, j+1, n1, n2, s, p) || isMatch(i+1, j, n1, n2, s, p);
            }
            return dp[i][j] =  false;
        }
