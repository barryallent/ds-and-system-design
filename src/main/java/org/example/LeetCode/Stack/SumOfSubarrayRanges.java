...
public class SumOfSubarrayRanges {
    ...
    // basically we will look in how many subarrays ith element can occur as a
    // minimum
    // idea would be to get previous smaller element index and next smaller element
    // index
    // now the element would be included in arrays till previous smaller index to
    // next smaller index
    public long sumSubarrayMins(int[] arr) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // total subarrays formed that has ith element as smallerst is
            // leftside*rightside
            long totalTimesIOccur = (long) leftSide * rightSide;

            // get contribution of i from total subarrays formed from i
            long totalIContribution = totalTimesIOccur * arr[i];

            // add contribution of i to total contribution
            totalContribution = (totalContribution + totalIContribution);
        }

        return totalContribution;
    }

    // get next larger elements for elements of arr
    int[] getNextLargerElement(int[] arr, Stack<int[]> s1) {
        int n = arr.length;
        int[] nextLargerElement = new int[n];
        Arrays.fill(nextLargerElement, -1);
        for (int i = 0; i < n; i++) {
            while (!s1.isEmpty() && s1.peek()[1] < arr[i]) {
                int[] topElement = s1.pop();
                nextLargerElement[topElement[0]] = i;
            }
            s1.push(new int[] { i, arr[i] });
        }
    ...
    // get previous larger elements for elements of arr
    int[] getPreviousLargerElement(int[] arr, Stack<int[]> s1) {
        int n = arr.length;
        int[] previousLargerElement = new int[n];

        Arrays.fill(previousLargerElement, -1);

        for (int i = 0; i < n; i++) {
            while (!s1.isEmpty() && s1.peek()[1] <= arr[n - i - 1]) {
                int[] topElement = s1.pop();
                previousLargerElement[topElement[0]] = n - i - 1;
            }
            s1.push(new int[] { n - i - 1, arr[n - i - 1] });
        }
        return previousLargerElement;
    }

    // basically we will look in how many subarrays ith element can occur as a
    // maximum
    // idea would be to get previous larger element index and next larger element
    // index
    // now the element would be included in arrays till previous larger index to
    // next larger index
    public long sumSubarrayMaxs(int[] arr) {
        int n = arr.length;

        // get next larger elements indices
        Stack<int[]> s1 = new Stack<>();
        int[] nextLargerElement = getNextLargerElement(arr, s1);

        // get previous larger elements indices
        Stack<int[]> s2 = new Stack<>();
        int[] previousLargerElement = getPreviousLargerElement(arr, s2);
        ...
        for (int i = 0; i < n; i++) {
            ...
            // if we have no next larger element then we take all elements till right
            if (nextLargerElement[i] == -1) {
                rightSide = n - i;
            }
            // else take elements from i to next smaller index
            else {
                rightSide = nextLargerElement[i] - i;
            }

            // if we have no previous larger element then we take all elements till left
            if (previousLargerElement[i] == -1) {
                leftSide = i + 1;
            }

            // else take elements from prev smaller index to i
            else {
                leftSide = i - previousLargerElement[i];
            }
    ...
    public long subArrayRanges(int[] nums) {
        ...
        // answer would be sum of subarray maximums - sum of subarray minimums
